---
id: CHG-0001-adopt-specsync-5-0-1-and-trust-1-0-0-governance-for-the-jwt-fledge-plugin
state: draft
type: migration
base_commit: faebb03ef390ce022f58aca139c208941faed1ad
---

# Adopt SpecSync 5.0.1 and Trust 1.0.0 governance for the JWT Fledge plugin

## Intent

Adopt SpecSync 5.0.1 and Trust 1.0.0 governance for the JWT Fledge plugin

## Affected Canonical Specs

- `jwt`

## Acceptance Criteria

- SpecSync strict checks pass at the explicit advisory threshold 0 for the extensionless Bash executable; Claude
- Cursor
- Codex
- and Gemini integrations are installed; Trust doctor and verification pass; ShellCheck
- syntax
- help
- deterministic decode
- invalid-input
- and manifest checks remain green.

## No-spec Rationale

Not applicable
